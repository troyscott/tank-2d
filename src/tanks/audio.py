    ...
    def play(self, name: str) -> None:
        if name in self.paths and self.winsound:
            self.winsound.PlaySound(
                self.paths[name], self.winsound.SND_FILENAME | self.winsound.SND_ASYNC
            )
    ...
    def _init_mixer(self) -> None:
        ...
        if backend.init():
            ...
            if not isinstance(backend, PygameAudioBackend):
                print(
                    f"Native backend {backend.__class__.__name__} failed, falling back to PygameAudioBackend"
                )
